...
class ReportAPIController extends AppBaseController
{
    ...
    public function getProfitLossReport(Request $request)
    {
        ...
        $hasDate = !empty($startDate) && !empty($endDate) && $startDate !== 'null' && $endDate !== 'null';
        ...
        $salesPaymentQuery = SalesPayment::query();
        $salesItemsQuery = Sale::with('saleItems.product');
        ...
        if ($hasDate) {
            ...
            $purchaseReturnsQuery->whereBetween('date', [$startDate, $endDate]);
            $purchasesQuery->whereBetween('date', [$startDate, $endDate]);
            $saleReturnsQuery->whereBetween('date', [$startDate, $endDate]);
            $expensesQuery->whereBetween('date', [$startDate, $endDate]);
            $salesPaymentQuery->whereBetween('payment_date', [$startDate, $endDate]);
            $salesItemsQuery->whereBetween('date', [$startDate, $endDate]);
            $allSaleReturnsItemsQuery->whereBetween('sales.date', [$startDate, $endDate]);
        }

        $data = [];
        $data['sales'] = (float) $salesQuery->sum('grand_total');
        $data['purchase_returns'] = (float) $purchaseReturnsQuery->sum('grand_total');
        $data['purchases'] = (float) ($purchasesQuery->sum('grand_total') - $data['purchase_returns']);
        $data['sale_returns'] = (float) $saleReturnsQuery->sum('grand_total');
        $data['expenses'] = (float) $expensesQuery->sum('amount');
        $data['sales_payment_amount'] = (float) $salesPaymentQuery->sum('amount');
        ...
        $sales = $salesItemsQuery->get();
        $allSaleReturnsItems = $allSaleReturnsItemsQuery->get();
        ...
        foreach ($allSaleReturnsItems as $saleReturn) {
            if ($saleReturn->product) {
                $productItemCost += ($saleReturn->product->product_cost * $saleReturn->quantity);
            }
        }
