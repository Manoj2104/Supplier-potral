...
class SaleAPIController extends AppBaseController
{
    ...
    {
        $perPage = $request->get('pageSize') ?? $request->get('limit') ?? getPageSize($request);
        if ($request->has('page') && is_array($request->get('page')) && isset($request->get('page')['size'])) {
            $perPage = (int) $request->get('page')['size'];
        }
        ...
        if (!empty($search)) {
            $sales = $sales->where(function (Builder $query) use ($search) {
        ...
        $sales = $sales->with(['customer', 'warehouse', 'user', 'payments', 'saleItems.product'])->orderBy('id', 'desc')->paginate($perPage);
    ...
    {
        if (!$sale) {
            throw new UnprocessableEntityHttpException('Sale not found');
        }
    ...
    public function destroy($id): JsonResponse
    {
        try {
            ...
            if ($sale && File::exists(Storage::path('sales/barcode-' . $sale->reference_code . '.png'))) {
