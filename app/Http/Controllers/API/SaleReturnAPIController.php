...
class SaleReturnAPIController extends AppBaseController
{
    ...
    public function index(Request $request): SaleReturnCollection
    {
        ...
        $salesReturn = \App\Models\SaleReturn::query();

        if (!empty($search)) {
            $salesReturn = $salesReturn->where(function (Builder $query) use ($search) {
        ...
        $salesReturn = $salesReturn->with(['customer', 'warehouse', 'saleReturnItems.product'])->latest('id')->paginate($perPage);
