...
class AdjustmentAPIController extends AppBaseController
{
    ...
    public function index(Request $request): AdjustmentCollection
    {
        ...
        $adjustments = \App\Models\Adjustment::query();

        if (!empty($search)) {
            $adjustments = $adjustments->where(function (Builder $query) use ($search) {
        ...
        $adjustments = $adjustments->with(['warehouse', 'adjustmentItems.product'])->latest('id')->paginate($perPage);
    ...
    public function destroy(int $id)
    {
        try {
            ...
            foreach ($adjustment->adjustmentItems as $adjustmentItem) {
                ...
                // Reverse adjustment effect in bin inventories and manage_stocks
                $reverseMethod = ($oldItem->method_type == AdjustmentItem::METHOD_ADDITION) 
                    ? AdjustmentItem::METHOD_SUBTRACTION 
                    : AdjustmentItem::METHOD_ADDITION;

                \App\Repositories\AdjustmentRepository::syncBinInventoryForAdjustment(
                    $oldItem->product_id,
                    $oldItem->quantity,
                    $reverseMethod,
