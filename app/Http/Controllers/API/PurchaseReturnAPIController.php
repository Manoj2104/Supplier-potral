...
class PurchaseReturnAPIController extends AppBaseController
{
    ...
    public function index(Request $request): PurchaseReturnCollection
    {
        ...
        $purchasesReturn = \App\Models\PurchaseReturn::query();

        if (!empty($search)) {
            $purchasesReturn = $purchasesReturn->where(function (Builder $query) use ($search) {
        ...
        $purchasesReturn = $purchasesReturn->with(['supplier', 'warehouse', 'purchaseReturnItems.product'])->latest('id')->paginate($perPage);
