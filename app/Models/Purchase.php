...
class Purchase extends BaseModel implements HasMedia, JsonResourceful
{
    ...
    {
        $asn = SupplierAsn::where('purchase_id', $this->id)->latest('id')->first();

        $items = $this->purchaseItems ? $this->purchaseItems->map(function ($item) {
            $product = $item->product;
            $imgUrl = $product ? ($product->image_url ?? null) : null;
        ...
        $fields = [
            ...
            'asn' => $asn ? [
                ...
                'driver_mobile' => $asn->driver_mobile,
                'transport_company' => $asn->transport_company,
                'lr_number' => $asn->lr_number,
                'invoice_number' => $asn->invoice_number,
                'eway_bill' => $asn->eway_bill,
                'dispatch_date' => $asn->dispatch_date ? $asn->dispatch_date->format('Y-m-d') : null,
                'expected_arrival' => $asn->expected_arrival ? $asn->expected_arrival->format('Y-m-d') : null,
                'status' => $asn->status,
            ] : null,
        ];
    ...
    public function asn(): \Illuminate\Database\Eloquent\Relations\HasOne
    {
        return $this->hasOne(\App\Models\SupplierAsn::class, 'purchase_id', 'id');
    }

    public function cartons(): HasMany
    {
        return $this->hasMany(\App\Models\LpnCarton::class, 'purchase_id', 'id');
    }
