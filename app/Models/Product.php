...
class Product extends BaseModel implements HasMedia, JsonResourceful
{
    ...
    {
        $relations = [];
        if (!$this->relationLoaded('variationProduct')) {
            $relations[] = 'variationProduct.variation';
            $relations[] = 'variationProduct.variationType';
        }
        if (!$this->relationLoaded('mainProduct')) {
        ...
        if (static::$cachedWarehouseId === null) {
            $warehouse = \App\Models\Warehouse::first();
            static::$cachedWarehouseId = $warehouse ? $warehouse->id : 1;
        }
        $whId = static::$cachedWarehouseId;

        if ($this->relationLoaded('stock') && $this->stock !== null) {
            $binStock = (float) $this->stock->quantity;
        } else {
            $binStock = (float) \Illuminate\Support\Facades\DB::table('bin_inventories')
                ->join('warehouse_bins', 'bin_inventories.bin_code', '=', 'warehouse_bins.bin_code')
                ->where('warehouse_bins.is_active', 1)
                ->where('bin_inventories.product_id', $this->id)
                ->sum('bin_inventories.quantity');

            $stockModel = \App\Models\ManageStock::updateOrCreate(
                ['product_id' => $this->id, 'warehouse_id' => $whId],
                ['quantity' => $binStock]
            );
            $this->setRelation('stock', $stockModel);
        }
    ...
    protected static $cachedBaseUnits = null;
    protected static $cachedUnits = null;
    protected static $cachedWarehouseId = null;
    ...
    {
        if (self::$cachedBaseUnits === null) {
            self::$cachedBaseUnits = BaseUnit::all()->keyBy('id');
        }
        if (self::$cachedUnits === null) {
            self::$cachedUnits = Unit::all()->keyBy('id');
        }
        $productUnit = self::$cachedBaseUnits[$this->product_unit] ?? (self::$cachedUnits[$this->product_unit] ?? null);
        if ($productUnit) {
    ...
    public function getPurchaseUnitName()
    {
        ...
        $purchaseUnit = self::$cachedUnits[$this->purchase_unit] ?? null;
        if ($purchaseUnit) {
    ...
    public function getSaleUnitName()
    {
        ...
        $saleUnit = self::$cachedUnits[$this->sale_unit] ?? null;
        if ($saleUnit) {
    ...
    {
        $medias = $this->getMedia(Product::PATH);
        if ($medias->isEmpty() && $this->mainProduct) {
            $medias = $this->mainProduct->getMedia('main_product');
        }
        if ($medias->isEmpty() && $this->mainProduct) {
            $medias = $this->mainProduct->getMedia(Product::PATH);
        }
        ...
        return [
            ...
            'image' => $urls,
            'category_name' => $this->productCategory ? $this->productCategory->name : ($this->mainProduct && $this->mainProduct->productCategory ? $this->mainProduct->productCategory->name : 'General'),
        ];
    }
     ...
     * @return mixed
     */
    protected static $cachedWarehouseStocks = null;
    protected static $cachedInStock = null;

    public function warehouse($id)
    {
        if (self::$cachedWarehouseStocks === null) {
            self::$cachedWarehouseStocks = ManageStock::join('warehouses', 'manage_stocks.warehouse_id', '=', 'warehouses.id')
                ->select('manage_stocks.product_id', 'manage_stocks.quantity as total_quantity', 'warehouses.name')
                ->get()
                ->groupBy('product_id');
        }
        return self::$cachedWarehouseStocks[$id] ?? collect();
    }
    ...
    public function inStock($id)
    {
        $binStock = (float) \Illuminate\Support\Facades\DB::table('bin_inventories')
            ->join('warehouse_bins', 'bin_inventories.bin_code', '=', 'warehouse_bins.bin_code')
            ->where('warehouse_bins.is_active', 1)
            ->where('bin_inventories.product_id', $id)
            ->sum('bin_inventories.quantity');

        $warehouse = \App\Models\Warehouse::first();
        $whId = $warehouse ? $warehouse->id : 1;
        \App\Models\ManageStock::updateOrCreate(
            ['product_id' => $id, 'warehouse_id' => $whId],
            ['quantity' => $binStock]
        );
        self::$cachedInStock[$id] = $binStock;
