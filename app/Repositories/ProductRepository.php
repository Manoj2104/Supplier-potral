...
class ProductRepository extends BaseRepository
{
    ...
    public function storeProduct($input)
    {
        try {
            ...
            try {
                ...
                \Log::warning('Barcode PNG generation notice: ' . $bcErr->getMessage());
        ...
        if (isset($input['purchase_quantity'])) {
            ...
            if ($purchaseStock) {
                ...
                $poCode = 'PO-' . date('Y') . '-' . str_pad($purchase->id, 6, '0', STR_PAD_LEFT);
                $purchase->update([
                    'reference_code' => $poCode,
                ...
                // Auto-allocate Opening Stock into primary WMS bin for instant Available For Sale inventory
                try {
                    $defaultBin = DB::table('warehouse_bins')->where('warehouse_id', $purchaseStock['warehouse_id'])->first();
                    $binId = $defaultBin ? $defaultBin->id : 1;
                    
                    DB::table('bin_inventories')->updateOrInsert(
                        ['warehouse_id' => $purchaseStock['warehouse_id'], 'bin_id' => $binId, 'product_id' => $product->id],
                        ['quantity' => $purchaseStock['quantity'], 'updated_at' => now(), 'created_at' => now()]
                    );

                    // Auto-create Supplier ASN & Putaway completion record so it updates across all WMS screens
                    \App\Models\SupplierAsn::create([
                        'purchase_id' => $purchase->id,
                        'supplier_id' => $purchaseStock['supplier_id'],
                        'asn_number' => 'ASN-' . date('Y') . '-' . str_pad($purchase->id, 5, '0', STR_PAD_LEFT),
                        'status' => 'putaway_completed',
                        'expected_delivery_date' => now(),
                        'vehicle_number' => 'IN-HOUSE-OPENING',
                    ]);
                } catch (\Exception $ex) {
                    // Ignore if bin/ASN table structure varies
                }
            }
        }

            // Save Product Media (Uploaded files or Remote URLs)
            $imagesToProcess = [];
            if (!empty($input['images'])) {
                $imagesToProcess = is_array($input['images']) ? $input['images'] : [$input['images']];
            } elseif (!empty($input['image'])) {
                $imagesToProcess = [$input['image']];
            } elseif (!empty($input['image_url'])) {
                $imagesToProcess = [$input['image_url']];
            }
            ...
            foreach ($imagesToProcess as $img) {
                ...
                try {
                    if ($img instanceof \Illuminate\Http\UploadedFile) {
                        $product->addMedia($img)->toMediaCollection(Product::PATH, config('app.media_disc'));
                    } elseif (is_string($img)) {
                        if (str_starts_with($img, 'http://') || str_starts_with($img, 'https://')) {
                            $product->addMediaFromUrl($img)->toMediaCollection(Product::PATH, config('app.media_disc'));
                        } elseif (str_starts_with($img, 'data:image')) {
                            $product->addMediaFromBase64($img)->toMediaCollection(Product::PATH, config('app.media_disc'));
                        }
                    }
                } catch (\Throwable $e) {
                    \Log::warning('Product media save: ' . $e->getMessage());
                }
            }
    ...
    public function updateProduct($input, $id)
    {
        try {
            ...
            if (isset($input['images']) || isset($input['image']) || isset($input['image_url'])) {
                $imagesToProcess = [];
                if (!empty($input['images'])) {
                    $imagesToProcess = is_array($input['images']) ? $input['images'] : [$input['images']];
                } elseif (!empty($input['image'])) {
                ...
                if (!empty($imagesToProcess)) {
                    ...
                    foreach ($imagesToProcess as $img) {
                        ...
                        try {
                            ...
                            \Log::warning('Product media update: ' . $e->getMessage());
                        }
                    }
                }
            }
    ...
    public function generateBarcode($input, $reference_code): bool
    {
        try {
            ...
            \Log::warning('Barcode PNG generator exception: ' . $e->getMessage());
            return false;
        }
    }
}
