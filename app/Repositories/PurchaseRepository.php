...
class PurchaseRepository extends BaseRepository
{
    ...
    public function storePurchase($input)
    {
        try {
            ...
            // Note: Purchase Order creation does NOT add to available stock.
            // Stock is only added after PDA Receiving and Putaway is completed into physical bins.

            DB::commit();

            $purchaseId = $purchase->id;
            // Dispatch Email Asynchronously in Background so HTTP response completes INSTANTLY (100ms)
            try {
                $artisanPath = base_path('artisan');
                $phpBinary = PHP_BINARY ?: 'C:\\xampp\\php\\php.exe';
                if (str_starts_with(strtoupper(PHP_OS), 'WIN')) {
                    pclose(popen("start /B \"\" \"{$phpBinary}\" \"{$artisanPath}\" send:po-email {$purchaseId} > NUL 2>&1", "r"));
                } else {
                    exec("\"{$phpBinary}\" \"{$artisanPath}\" send:po-email {$purchaseId} > /dev/null 2>&1 &");
                }
            } catch (\Exception $asyncEx) {
                \Log::warning("Async PO email trigger warning: " . $asyncEx->getMessage());
            }
        ...
        try {
            ...
            foreach ($input['purchase_items'] as $key => $purchaseItem) {
                ...
                if (is_null($purchaseItem['purchase_item_id'])) {
                    ...
                    // Note: Stock managed via Putaway
