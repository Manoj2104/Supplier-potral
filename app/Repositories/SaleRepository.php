...
class SaleRepository extends BaseRepository
{
    ...
    public function storeSale($input): Sale
    {
        try {
            ...
            $reference_code = rtrim(getSettingValue('sale_code'), '_-').'-111'.$sale->id;
    ...
    public function storeSaleItems($sale, $input)
    {
        foreach ($input['sale_items'] as $saleItem) {
            ...
            if (! empty($product) && ! empty($product->quantity_limit) && (int)$product->quantity_limit > 0 && $saleItem['quantity'] > (int)$product->quantity_limit) {
        ...
        $input['reference_code'] = rtrim(getSettingValue('sale_code'), '_-').'-111'.$sale->id;
        $sale->update($input);
