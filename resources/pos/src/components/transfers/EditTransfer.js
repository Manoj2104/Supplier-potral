...
const EditTransfer = ( props ) => {
    ...
    const transferStatusDefaultValue = tansfers && tansfers.attributes && tansfers.attributes.status && transferStatusFilterOptions.filter( ( item ) => item.id === tansfers.attributes.status );
    ...
    const itemsValue = tansfers && tansfers.attributes && {
        ...
        status_id: {
            label: transferStatusDefaultValue && transferStatusDefaultValue[ 0 ] && transferStatusDefaultValue[ 0 ].name,
            value: transferStatusDefaultValue && transferStatusDefaultValue[ 0 ] && transferStatusDefaultValue[ 0 ].id
        },
    };
    ...
    return (
        <MasterLayout>
            {isLoading ? <Spinner /> :
                <TransferForm singleTransfer={itemsValue} id={id} warehouses={warehouses} isEdit={true} />}
        </MasterLayout>
    );
};
