...
const ProductDetail = (props) => {
    ...
    const barcode = (mainSubProduct.code && /^\d{6,16}$/.test(String(mainSubProduct.code).trim()))
        ? String(mainSubProduct.code).trim()
        : (mainSubProduct.code || mainAttr.code || "8901898053777");
    const productCode = (mainSubProduct.product_code && !/^\d{12,14}$/.test(String(mainSubProduct.product_code).trim()))
        ? String(mainSubProduct.product_code).trim()
        : (mainAttr.code || "SKU-PRD-001");
