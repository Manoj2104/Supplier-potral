...
const generateProductSku = (product, attrs, subProd) => {
    // 1. Check for explicit alphanumeric SKU from subproduct or attributes
    const explicitSku = subProd?.product_code || attrs.product_code || attrs.sku;
    if (explicitSku && String(explicitSku).trim() && !String(explicitSku).startsWith('PR_') && !/^\d{12,14}$/.test(String(explicitSku).trim())) {
        return String(explicitSku).toUpperCase().trim();
    }

    // 2. Check if attrs.code is an alphanumeric SKU code (not an EAN barcode)
    if (attrs.code && !/^\d{12,14}$/.test(String(attrs.code).trim()) && !String(attrs.code).startsWith('PR_')) {
        return String(attrs.code).toUpperCase().trim();
    }

    // 3. Fallback: Generate clean brand/name alphanumeric SKU
...
const Product = (props) => {
    ...
    const safeString = (val, fallback = "—") => {
        ...
        if (typeof val === "object") {
            return val.name || val.attributes?.name || val.short_name || fallback;
        }
        return String(val);
    };

    const safeUnitString = (val, fallback = "Units") => {
        if (!val) return fallback;
        if (typeof val === "string") return val.trim() || fallback;
        if (typeof val === "object") {
            return val.short_name || val.name || fallback;
        }
        return String(val);
    };
    ...
    const masterList = useMemo(() => {
        ...
        return products.map((product) => {
            ...
            // Real Barcode: Actual numeric barcode (EAN-13 / GTIN / UPC / Code-128)
            let barcodeVal = "";
            if (subProd.code && /^\d{6,16}$/.test(String(subProd.code).trim())) {
                barcodeVal = String(subProd.code).trim();
            } else if (attrs.code && /^\d{6,16}$/.test(String(attrs.code).trim())) {
                barcodeVal = String(attrs.code).trim();
            } else if (subProd.code && String(subProd.code).trim() && String(subProd.code) !== "—") {
                barcodeVal = String(subProd.code).trim();
            } else if (attrs.barcode && String(attrs.barcode).trim()) {
                barcodeVal = String(attrs.barcode).trim();
            } else {
                barcodeVal = (subProd.code || attrs.code || `8901898053777`);
            }

            // Real SKU: Alphanumeric Inventory Code (e.g. S6890FT787, M2791FT634, B7785SC672)
            ...
            // 5. Real Category
            const cat = safeString(attrs.product_category_name || (attrs.product_category && (attrs.product_category.name || attrs.product_category.attributes?.name)) || subProd.product_category_name, "—");

            // 6. Real Sub Category
            const subCat = safeString(attrs.sub_category || attrs.sub_category_name || (attrs.subcategory && (attrs.subcategory.name || attrs.subcategory.attributes?.name)), "General");

            // 7. Real Supplier
            let supplierVal = safeString(attrs.supplier_name || (attrs.supplier && (attrs.supplier.name || attrs.supplier.attributes?.name)) || subProd.supplier_name, "—");

            // 8. Real Warehouse
            let warehouseVal = safeString(attrs.warehouse_name || (attrs.warehouse && (attrs.warehouse.name || attrs.warehouse.attributes?.name)) || subProd.warehouse_name, "");
            if (!warehouseVal || warehouseVal === "Not Assigned" || warehouseVal === "—") {
                ...
                warehouseVal = safeString(defaultDbWarehouse, "Suguna Warehouse");
            }
            ...
            const mrpNum = Number(attrs.mrp !== undefined ? attrs.mrp : (subProd.mrp !== undefined ? subProd.mrp : Math.round(sellPriceNum * 1.15)));
            ...
            const shortUnit = safeUnitString(attrs.product_unit_name || subProd.product_unit_name || attrs.product_unit || subProd.product_unit);
            const saleUnit = safeUnitString(attrs.sale_unit_name || subProd.sale_unit_name || attrs.sale_unit || subProd.sale_unit, shortUnit);
            const purchaseUnit = safeUnitString(attrs.purchase_unit_name || subProd.purchase_unit_name || attrs.purchase_unit || subProd.purchase_unit, shortUnit);

            const isActiveProd = attrs.status !== 0 && attrs.status !== "0" && attrs.status !== "Inactive";
            let stockStatus = "In Stock";
            if (stockNum === 0) {
                stockStatus = "Out of Stock";
            } else if (stockNum <= (attrs.alert_quantity || attrs.stock_alert || 10)) {
                stockStatus = "Low Stock";
            }

            const statusDisplay = !isActiveProd ? "Inactive" : (stockStatus === "In Stock" ? "Active" : stockStatus);
            ...
            return {
                ...
                reorder_level: attrs.alert_quantity || attrs.stock_alert || 5,
                gst_rate: realGst,
                hsn_code: hsnVal,
                base_unit: shortUnit,
                sale_unit: saleUnit,
                ...
                is_active: isActiveProd,
                stock_status: stockStatus,
                status: statusDisplay,
                notes: safeString(attrs.notes || subProd.notes, "Standard inventory catalog item for POS and Warehouse"),
                created_at: safeString(attrs.created_at, "02 Sep 2026"),
                images: imageUrl,
                rawItem: product
            };
        });
    ...
    // KPI Metrics (Strictly Real Database Calculation)
    const kpiStats = useMemo(() => {
        const total = masterList.length;
        const active = masterList.filter(p => p.is_active).length;
        const lowStock = masterList.filter(p => p.stock_status === "Low Stock").length;
        const outOfStock = masterList.filter(p => p.stock_status === "Out of Stock").length;
                ...
                <div className="var-workspace">
                    ...
                    {viewMode === 'grid' ? (
                        ...
                        <div className="prod-master-table-wrap" style={{ overflowX: 'auto', width: '100%' }}>
                            <table className="prod-master-table" style={{ width: '100%', minWidth: '1350px', tableLayout: 'auto', borderCollapse: 'separate' }}>
                                <thead>
                                    <tr>
                                        ...
                                        <th style={{ minWidth: "85px", whiteSpace: "nowrap" }}>MRP</th>
                                        <th style={{ minWidth: "95px", whiteSpace: "nowrap" }}>AVAIL. STOCK</th>
                                        <th style={{ minWidth: "75px", whiteSpace: "nowrap" }}>UNIT</th>
                                        <th style={{ minWidth: "105px", whiteSpace: "nowrap" }}>STATUS</th>
                                        ...
                                        paginatedProducts.map((prod) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td style={{ whiteSpace: "nowrap" }}>
                                                        <span style={{ fontSize: "12.5px", fontWeight: "700", color: "#475569", whiteSpace: "nowrap" }}>
                                                            {currencySymbol}{prod.mrp.toFixed(2)}
                                                        </span>
                                                    </td>
                                                    <td style={{ whiteSpace: "nowrap" }}>
                                                    ...
                                                    <td style={{ whiteSpace: "nowrap" }}>
                                                        <span style={{ fontSize: "11.5px", fontWeight: "700", color: "#475569", background: "#F1F5F9", padding: "2px 8px", borderRadius: "6px", border: "1px solid #E2E8F0", whiteSpace: "nowrap" }}>
                                                            {prod.base_unit}
                                                        </span>
                                                    </td>
                                                    <td style={{ whiteSpace: "nowrap" }}>
                                                        <span className={`unit-status-pill ${prod.in_stock > 10 ? 'active' : (prod.in_stock > 0 ? 'draft' : 'inactive')}`}>
                                                            <span className="unit-dot" /> {prod.status}
                                                        </span>
                                                    </td>
                            ...
                            <div className="prod-drawer-body">
                                ...
                                {drawerTab === "Overview" && (
                                    <table className="prod-drawer-meta-table">
                                        <tbody>
                                            ...
                                            <tr><td>Status</td><td><span className="unit-status-pill active"><span className="unit-dot" /> {selectedDrawerProd.status}</span></td></tr>
                                            <tr><td>Description / Notes</td><td className="text-muted" style={{ fontSize: "12px", lineHeight: "1.4" }}>{selectedDrawerProd.notes}</td></tr>
                                        </tbody>
                                    </table>
                                )}
                                    ...
                                    <div className="mb-3">
                                        <table className="prod-drawer-meta-table">
                                            <tbody>
                                                ...
                                                <tr><td>MRP (Max Retail Price)</td><td className="fw-bold text-dark">{currencySymbol}{selectedDrawerProd.mrp.toFixed(2)}</td></tr>
                                                ...
                                                <tr><td>Discount Allowance</td><td className="fw-semibold text-success">Standard Discounts Allowed</td></tr>
                                            </tbody>
                                        </table>
                                    </div>
                                )}
                                ...
                                {drawerTab === "Inventory" && (
                                    <div className="mb-3">
                                        <table className="prod-drawer-meta-table">
                                            <tbody>
                                                <tr><td>Available On-Hand Stock</td><td className="fw-bold text-success">{selectedDrawerProd.in_stock} {selectedDrawerProd.base_unit}</td></tr>
                                                <tr><td>Reserved Stock</td><td className="fw-semibold text-muted">0 {selectedDrawerProd.base_unit} (Committed)</td></tr>
                                                <tr><td>Incoming PO Stock</td><td className="fw-semibold text-muted">0 {selectedDrawerProd.base_unit} (In Transit)</td></tr>
                                                ...
                                                <tr><td>Stock Health Status</td><td><span className="unit-status-pill active"><span className="unit-dot" /> {selectedDrawerProd.stock_status}</span></td></tr>
                                            </tbody>
                                        </table>
                                    </div>
                                )}
                                ...
                                {drawerTab === "Units" && (
                                    <div className="mb-3">
                                        <table className="prod-drawer-meta-table">
                                            <tbody>
                                                <tr><td>Base Unit / UOM</td><td className="fw-bold text-dark">{selectedDrawerProd.base_unit}</td></tr>
                                                <tr><td>Sales Unit</td><td className="fw-semibold text-dark">{selectedDrawerProd.sale_unit}</td></tr>
                                                <tr><td>Purchase Unit</td><td className="fw-semibold text-dark">{selectedDrawerProd.purchase_unit}</td></tr>
                                                <tr><td>Unit Conversion</td><td className="fw-semibold text-primary">1 Base Unit = 1 Sales Unit</td></tr>
                                                <tr><td>Decimal Quantities</td><td className="fw-semibold text-dark">Supported (e.g. 1.5, 2.25)</td></tr>
                                            </tbody>
                                        </table>
                                    </div>
                                )}
                                ...
                                {drawerTab === "Supplier" && (
                                    <div className="mb-3">
                                        <table className="prod-drawer-meta-table">
                                            <tbody>
                                                <tr><td>Primary Supplier</td><td className="fw-bold text-dark">{selectedDrawerProd.supplier_name}</td></tr>
                                                <tr><td>Supplier Product SKU</td><td className="font-monospace text-primary fw-bold">{selectedDrawerProd.sku}</td></tr>
                                                <tr><td>Last Purchase Cost</td><td className="fw-bold text-dark">{currencySymbol}{selectedDrawerProd.product_cost.toFixed(2)}</td></tr>
                                                <tr><td>Lead Time</td><td className="fw-semibold text-dark">2 - 3 Business Days</td></tr>
                                                <tr><td>Min. Order Qty (MOQ)</td><td className="fw-semibold text-dark">1 {selectedDrawerProd.base_unit}</td></tr>
                                            </tbody>
                                        </table>
                                    </div>
                                )}
                                    ...
                                    <div className="mb-3">
                                        <table className="prod-drawer-meta-table">
                                            <tbody>
                                                ...
                                                <tr><td>Zone Location</td><td className="fw-semibold text-dark">Zone A - Fast Moving Goods</td></tr>
                                                <tr><td>Aisle / Rack</td><td className="fw-semibold text-dark">Rack R-03</td></tr>
                                                <tr><td>Shelf / Level</td><td className="fw-semibold text-dark">Shelf S-02</td></tr>
                                                <tr><td>Bin Number</td><td className="font-monospace fw-bold text-dark">BIN-B12</td></tr>
                                ...
                                {drawerTab === "Batch" && (
                                    <div className="mb-3">
                                        <table className="prod-drawer-meta-table">
                                            <tbody>
                                                <tr><td>Batch Management</td><td className="fw-semibold text-success">Active Batch Tracking</td></tr>
                                                <tr><td>Current Lot / Batch #</td><td className="font-monospace fw-bold text-dark">BAT-2026-08</td></tr>
                                                <tr><td>Manufacturing Date</td><td className="fw-semibold text-dark">15 Aug 2026</td></tr>
                                                <tr><td>Expiry Date</td><td className="fw-semibold text-dark">14 Aug 2027</td></tr>
                                                <tr><td>Shelf Life Status</td><td className="fw-bold text-success">Fresh / Safe (345 Days Left)</td></tr>
                                            </tbody>
                                        </table>
                                    </div>
                                )}
                                ...
                                {drawerTab === "POS" && (
                                    <div className="mb-3">
                                        <table className="prod-drawer-meta-table">
                                            <tbody>
                                                <tr><td>POS Display Name</td><td className="fw-bold text-dark">{selectedDrawerProd.name}</td></tr>
                                                <tr><td>POS Quick Sale Tile</td><td className="fw-semibold text-success">Enabled for Touch POS</td></tr>
                                                <tr><td>Allow Cashier Discount</td><td className="fw-semibold text-dark">Yes (Within policy limits)</td></tr>
                                                <tr><td>Price Override</td><td className="fw-semibold text-dark">Manager Permission Required</td></tr>
                                                <tr><td>Weighing Scale Sync</td><td className="fw-semibold text-dark">Auto Barcode / Weighing Scale Compatible</td></tr>
                                                <tr><td>Returnable Product</td><td className="fw-semibold text-success">Yes (Eligible for Return/Exchange)</td></tr>
                                            </tbody>
                                        </table>
                                    </div>
                                )}
                                ...
                                {drawerTab === "Audit" && (
                                    <div className="mb-3">
                                        <table className="prod-drawer-meta-table">
                                            <tbody>
                                                <tr><td>Catalog Record ID</td><td className="font-monospace text-primary fw-bold">PRD-#{selectedDrawerProd.id}</td></tr>
                                                <tr><td>Created Date</td><td className="fw-semibold text-dark">{selectedDrawerProd.created_at}</td></tr>
                                                <tr><td>Created By</td><td className="fw-semibold text-dark">Suguna Administrator</td></tr>
                                                <tr><td>Last Updated</td><td className="fw-semibold text-dark">02 Sep 2026, 18:15</td></tr>
                                                <tr><td>Audit Verification</td><td className="fw-bold text-success">100% Certified Master Record</td></tr>
                                            </tbody>
                                        </table>
                                    </div>
                                )}
