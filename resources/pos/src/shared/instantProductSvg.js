...
export const getInstantProductImage = (images, productName = "", categoryName = "") => {
    if (images) {
        if (typeof images === "string" && images.trim().length > 0 && !images.includes("brand_logo.png") && !images.includes("media-amazon.com")) {
            return images;
        }
        if (typeof images === "object") {
            if (images.image_url && typeof images.image_url === "string" && !images.image_url.includes("brand_logo.png") && !images.image_url.includes("media-amazon.com")) {
            ...
            const validUrls = urls.filter(u => typeof u === "string" && u.trim().length > 0 && !u.includes("brand_logo.png") && !u.includes("media-amazon.com"));
