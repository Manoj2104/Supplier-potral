...
export const addSmsTemplate = (smsTemplate, navigate) => async (dispatch) => {
    dispatch(setLoading(true));
    apiConfig
        .post(apiBaseURL.SMS_TEMPLATES, smsTemplate)
        .then((response) => {
            dispatch(setLoading(false));
            dispatch({
                type: smsTemplatesActionType.ADD_SMS_TEMPLATE || 'ADD_SMS_TEMPLATE',
                payload: response.data.data,
            });
            dispatch(
                addToast({
        ...
        .catch(({ response }) => {
            ...
            dispatch(
                addToast({
                    text: response?.data?.message || 'Error creating SMS template',
                    type: toastType.ERROR,
                })
            );
        });
