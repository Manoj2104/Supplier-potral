...
export const addEmailTemplate = (emailTemplate, navigate) => async (dispatch) => {
    dispatch(setLoading(true));
    apiConfig
        .post(apiBaseURL.EMAIL_TEMPLATES, emailTemplate)
        .then((response) => {
            dispatch(setLoading(false));
            dispatch({
                type: emailTemplatesActionType.ADD_EMAIL_TEMPLATE || 'ADD_EMAIL_TEMPLATE',
                payload: response.data.data,
            });
            dispatch(
                addToast({
        ...
        .catch(({ response }) => {
            ...
            dispatch(
                addToast({
                    text: response?.data?.message || 'Error creating email template',
                    type: toastType.ERROR,
                })
            );
        });
